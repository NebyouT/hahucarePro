...
class BodyChartController extends GetxController {
  ...
  TextEditingController templateCont = TextEditingController();
  ...
  Rx<BodyChartTemplate?> selectedTemplate = Rx<BodyChartTemplate?>(null);

  RxList<BodyChartTemplate> templateList = RxList();

  Future<void> getBodyChartTemplates() async {
    CoreServiceApis.getBodyChartTemplates(templateList: templateList).then((value) {}).catchError((e) {
      toast(e.toString());
    });
  }

  void onTemplateSelect(BodyChartTemplate? template) {
    if (template != null) {
      selectedTemplate(template);
      imageTitleCont.text = template.title;
      templateCont.text = template.title;
      bodyImage(template.image);
      editedImg(File(""));
