...
class AddPrescriptionComponentPharma extends StatelessWidget {
  ...
  AddPrescriptionComponentPharma({super.key, this.isEdit = false, this.index}) {
    ...
    if (isEdit && index != null) {
      ...
      final prescription = controller.prescriptionList[index!];
      final formData = PrescriptionFormData(
        medicine: prescription.medicine,
        frequency: prescription.frequency,
      );
      formData.durationCont.text = prescription.duration;
      formData.quantityCont.text = prescription.quantity.toString();
      formData.instructionCont.text = prescription.instruction;
      prescriptionFormDataList.add(formData);
    } else {
      prescriptionFormDataList.clear();

      if (controller.selectedMedicines.isNotEmpty) {
        for (var med in controller.selectedMedicines) {
          final existingFormData = prescriptionFormDataList.where((p) => p.medicine.id == med.id);
          if (existingFormData.isEmpty) {
            final existingPrescription = controller.prescriptionList.where((p) => p.medicine.id == med.id).firstOrNull;
            ...
            if (existingPrescription != null) {
              ...
              formData.durationCont.text = existingPrescription.duration;
              formData.quantityCont.text = existingPrescription.quantity.toString();
              formData.instructionCont.text = existingPrescription.instruction;
              prescriptionFormDataList.add(formData);
            } else {
  ...
  final ClinicalDetailsController clincalDetailCont = Get.find<ClinicalDetailsController>();
  ...
  Future<void> handleSelectMedicineTap() async {
    final result = await Get.to(
        ...
        arguments: {
          'medsAlreadyInPresc': <dynamic>{...prescriptionFormDataList.map((e) => e.medicine.id), ...clincalDetailCont.prescriptionList.map((e) => e.medicine.id)}.toList(),
